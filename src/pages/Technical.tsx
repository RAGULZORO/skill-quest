...
const Technical = () => {
  ...
  const fetchAnsweredQuestions = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select('question_id')
      .eq('user_id', user.id)
      .eq('question_type', 'technical_mcq');
  ...
  // Apply deterministic shuffle based on user ID and level
  const shuffledQuestions = selectedLevel && filteredQuestions.length > 0
    ? seededShuffle(
        filteredQuestions,
        createQuestionSeed(user?.id, 'technical', selectedLevel)
      )
    : filteredQuestions;
      ...
      <header className="sticky top-0 z-50 glass border-b border-border">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-lg font-bold text-foreground">Technical MCQs</span>
          </div>
      ...
      <main className="container mx-auto px-4 py-8">
        {!selectedLevel ? (
                ...
                return (
                  ...
                  >
                    ...
                    <p className="text-sm text-muted-foreground mt-2">
                      {questionsForLevel.length} questions
                    </p>
                  ...
                  <div className="space-y-2 mb-4">
                    {item.question.options.map((option, optIdx) => {
                      ...
                      return (
                        <div
                          key={optIdx}
                          className={`p-3 rounded-xl border ${optionStyle}`}
                        >
                          <div className="flex items-center gap-3">
          ...
          <div className="max-w-3xl mx-auto">
            ...
            {/* Question Card */}
            <div className="bg-card rounded-3xl shadow-card border border-border p-6 md:p-8 mb-6 animate-scale-in">
                ...
                {currentQuestion?.options.map((option, index) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <span className="w-8 h-8 rounded-full bg-background border border-border flex items-center justify-center text-sm font-medium">
        ...
        <section className="py-12 bg-gradient-hero">
          <div className="container mx-auto px-4 text-center">
            <h2 className="text-2xl font-bold text-primary-foreground mb-4">
              Ready to practice Group Discussion?
            </h2>
            <p className="text-primary-foreground/80 mb-6">
              Explore our GD topics and improve your communication skills.
            </p>
            <Button 
              onClick={() => navigate('/gd')}
